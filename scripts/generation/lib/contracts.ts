...
export const artifactNamespaceSchema = z.enum(["live", "dry_run"]);
export const importExecutionModeSchema = artifactNamespaceSchema;
...
export const attemptTerminalModeSchema = z.enum([
  "completed",
  "saturated",
  "failed",
  "preview_completed",
...
export const importWorkerDecisionStatusSchema = z.enum([
  "created",
  "rejected",
  "skipped_existing",
  "skipped_no_import",
...
export const importWorkerDecisionSchema = z.object({
  index: z.number().int().min(0),
  status: importWorkerDecisionStatusSchema,
  questionId: z.string().nullable(),
  reason: z.string().nullable(),
});

export const importWorkerReportSchema = z.object({
  ok: z.boolean(),
  mode: importExecutionModeSchema.default("live"),
  ...
  wouldImport: z.number().int().min(0).default(0),
  decisions: z.array(importWorkerDecisionSchema),
...
export const attemptHistoryEntrySchema = z.object({
  ...
  artifactNamespace: artifactNamespaceSchema.optional(),
  artifactRunId: z.string().min(1).nullable().optional(),
  importMode: importExecutionModeSchema.optional(),
...
export const batchAttemptResultArtifactPathsSchema = z.object({
  rawOutputPath: z.string().min(1),
  draftOutputPath: z.string().nullable().optional(),
  repairOutputPath: z.string().nullable().optional(),
  sourcePackPath: z.string().min(1),
  promptPath: z.string().min(1),
  overlapReportPath: z.string().min(1),
  semanticOverlapReportPath: z.string().min(1),
  validationReportPath: z.string().min(1),
  australianVerificationReportPath: z.string().min(1),
  mergedDecisionReportPath: z.string().min(1),
  importReportPath: z.string().min(1),
  attemptResultPath: z.string().min(1).optional(),
  batchResultPath: z.string().min(1).optional(),
  summaryPath: z.string().nullable().optional(),
});

export const batchAttemptResultSchema = z.object({
  workflowId: z.string().min(1),
  batchId: z.string().regex(/^B\d{2}$/),
  attemptNumber: z.number().int().min(1),
  terminalMode: attemptTerminalModeSchema,
  mode: batchAttemptModeSchema,
  importMode: importExecutionModeSchema,
  startedAt: z.string().datetime(),
  ...
  projectedAcceptanceSummary: z.object({
    acceptedTotalProjected: z.number().int().min(0),
    rejectedTotalProjected: z.number().int().min(0),
    remainingProjected: z.number().int().min(0),
  }).optional(),
  artifactPaths: batchAttemptResultArtifactPathsSchema,
  phase: z.string().min(1),
...
export type ArtifactNamespace = z.infer<typeof artifactNamespaceSchema>;
export type ImportExecutionMode = z.infer<typeof importExecutionModeSchema>;
...
export type BatchAttemptTerminalMode = z.infer<typeof attemptTerminalModeSchema>;
...
export type ImportWorkerDecision = z.infer<typeof importWorkerDecisionSchema>;
export type ImportWorkerReport = z.infer<typeof importWorkerReportSchema>;
