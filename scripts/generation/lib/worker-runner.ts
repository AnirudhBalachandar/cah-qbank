...
export const SEMANTIC_AUDITOR_TIMEOUT_MS = 20_000;
...
export type ImportWorkerReport = {
  ok: boolean;
  batchId: string;
  runId: string | null;
  ...
  decisions: ImportWorkerDecision[];
  error?: string | null;
};
...
export async function runImportWorker({
  ...
  try {
    ...
    return JSON.parse(raw) as ImportWorkerReport;
  } catch (error) {
