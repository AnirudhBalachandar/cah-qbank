...
import { buildReviewPackArtifactFileName } from "../scripts/generation/run_notes_workflow";
...
describe("workflow review-pack artifact writing", () => {
  ...
  it("uses a compact stable filename for large review-pack scopes", () => {
    const batchIds = Array.from({ length: 87 }, (_, index) => `B${String(index + 1).padStart(2, "0")}`);

    const fileName = buildReviewPackArtifactFileName(batchIds);

    expect(fileName).toMatch(/^review-pack-B01-to-B87-87-batches-[0-9a-f]{10}\.json$/);
